...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_out):
        ...
        #self.p_y_given_x=(p_y_given_x if activation is None else activation(p_y_given_x))#this is my addition
...
def load_data(dataset):
    ...
    print('now establishing data...')
    data_all = numpy.loadtxt(open('try1.txt'), delimiter=' ', skiprows=0)
    print(data_all[0:3])
    print('done1')
    target_all = numpy.loadtxt(open('try1_target.txt'), delimiter='\t', skiprows=0)
    print('done2')
    print(target_all[0:3])

    print('now data are splitting...')
    train_data = data_all[:800]
    train_target = target_all[:800]
    test_data = data_all[800:]
    test_target = target_all[800:]
    valid_data = data_all[800:]
    valid_target = target_all[800:]
